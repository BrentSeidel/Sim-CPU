...
private
   ...
   procedure decode_SUB(self : in out m68000)
      with pre => (not (self.instr.op2_size.code1 = 0 and self.instr.op2_size.code2 and
                   self.instr.op2_size.size /= data_long_long));
   procedure decode_SUBX(self : in out m68000)
      with pre => (self.instr.op2_size.code1 = 0 and self.instr.op2_size.code2 and
                   self.instr.op2_size.size /= data_long_long);
end;
