...
with BBS.Sim_CPU.io;
package BBS.Sim_CPU.io.serial.mux is
   --
   ...
   task type mux_server is
     entry start(self : mux_access; index : integer; port : GNAT.Sockets.Port_Type; owner : BBS.Sim_CPU.CPU.sim_access);
   ...
   --
   overriding
   procedure write(self : in out mux_tty; addr : addr_bus; data : data_bus; size : bus_size; status : in out bus_stat);
   --
   --  Read from a port address
   --
   overriding
   function read(self : in out mux_tty; addr : addr_bus; size : bus_size; status : in out bus_stat) return data_bus;
   --
...
private
   ...
   type mux_channel is limited record
      ...
      T         : BBS.sim_cpu.io.serial.mux.mux_server;
   end record;
