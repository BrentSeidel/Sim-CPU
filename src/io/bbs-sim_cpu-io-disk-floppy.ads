...
package BBS.Sim_CPU.io.disk.floppy is
   ...
   --
   overriding
   procedure write(self : in out hd_ctrl; addr : addr_bus; data : data_bus; size : bus_size; status : in out bus_stat);
   --
   --  Read from a port address
   --
   overriding
   function read(self : in out hd_ctrl; addr : addr_bus; size : bus_size; status : in out bus_stat) return data_bus;
   --
   --  How many addresses are used by the port
   --
   overriding
   ...
   overriding
   function dev_class(self : in out hd_ctrl) return BBS.Sim_CPU.io.dev_type is (HD);
   --
   --  Set which exception to use
   --
   overriding
