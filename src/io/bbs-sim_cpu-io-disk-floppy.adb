...
--
with Ada.Text_IO;
package body BBS.Sim_CPU.io.disk.floppy is
   --    0 - Control port
   --    1 - Sector number LSB
   --    2 - Sector number MSB
   --    3 - Track number LSB
   ...
   --
   overriding
   procedure write(self : in out fd_ctrl; addr : addr_bus; data : data_bus; size : bus_size; status : in out bus_stat) is
   ...
   --  7 - Not present
   overriding
   function read(self : in out fd_ctrl; addr : addr_bus; size : bus_size; status : in out bus_stat) return data_bus is
      offset    : constant byte := byte((addr - self.base) and 16#FF#);
