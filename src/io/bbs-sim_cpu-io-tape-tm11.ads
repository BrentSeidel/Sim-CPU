...
package BBS.Sim_CPU.io.tape.tm11 is
   ...
   --
   type tm11 is new ptape with private;
   type tm11_access is access all tm11'Class;
   --
   package tm11_io is new Ada.Sequential_IO(byte);
   --
   --  I/O device actions
   --
   ...
   --
   overriding
   function getSize(self : in out tm11) return addr_bus is (8);
   --
   --  Get device name/description
   --
   overriding
   function name(self : in out tm11) return string is ("MC");
   overriding
   ...
   --
   overriding
   procedure openIn(self : in out tm11; name : String);
   --
...
private
   --
   function read_tape(self : in out tm11) return data_bus;
   --
   --  Register addresses
   --
   ...
   --
   type tm11 is new ptape with record
