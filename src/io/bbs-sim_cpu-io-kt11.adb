         ...
         when bits8 =>
            ...
            case addr is
               ...
               when mmr2_sr2 + 1 =>
                  ...
                  ret_val := data_bus((self.mmr2/16#100#) and 16#FF#);
                  status := BUS_SUCC;
               when others =>
                  if self.host.trace.io or debug then
         ...
         when bits16 =>
            ...
            case addr is
               ...
               when mmr2_sr2 =>
                  ...
                  ret_val := data_bus(self.mmr2 and 16#FFFF#);
                  status := BUS_SUCC;
               when others =>
                  if self.host.trace.io or debug then
                     Ada.Text_IO.Put(" Unknown register");
...
--         Ada.Text_IO.Put_Line("MMU: Address " & toHex(addr) & " uses page number " & Integer'Image(num));
         case mode is
            when PROC_KERN =>
               cpdr := self.kid_pdr(num);
               if (cpdr.acf = 6) or ((cpdr.acf = 2) and not rw) then
...
--               Ada.Text_IO.Put_Line("MMU: Kernel Translated address is " & toHex(taddr));
               return taddr;
            when PROC_SUP =>
               if config.has_super then
                  null;
            ...
            when PROC_USER =>
               if config.has_user then
                  ...
                  taddr := taddr + cpar*16#40#;
--                  Ada.Text_IO.Put_Line("MMU: User Translated address is " & toHex(taddr));
                  return taddr;
               else
                  Ada.Text_IO.Put_Line("MMU: Supervisor processor mode not enabled.");
                  return bad_addr;
