...
Eighteen bit memory management is currently supported.
These processors do not have a separate I/O address space.
All I/O is mapped as part of the main memory.
...
\begin{description}
  \item[CPU] -- Shows the selected CPU and memory configuration.
  \item[INS-TALLED] -- Lists all the available CPUs and variants and all the I/O devices.
  \item[OP-TIONS $<$option-name$>$] -- Shows the value of a CPU specific option.
  \item[TR-ACE] -- Shows the current trace settings
\end{description}

\subsection{SET Subommands}
\label{cmd-set}
...
\begin{description}
  ...
  \begin{description}
    ...
    \item[PDP-11/34] -- A PDP-11 with EIS and 18 bit memory management.
