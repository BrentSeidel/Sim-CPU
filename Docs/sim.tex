...
\begin{description}
  \item[DL11] --  A single line serial interface.
  \item[KW11] -- A clock for the PDP-11 series of computer.
  \item[RK11] -- A disk controller that can have up to 8 attached RK05 simulated drives.
  \item[RK611] -- A disk controller than can have up to 8 attached RK07 simulated drives (RK06 drives are not supported).
  \item[PC11] -- A combination paper tape reader/punch.
  \item[MT11] -- A magnetic tape controller that can have up to 8 attached tape drives.  Note that this is internally treated as a disk drive so use the disk command to control it.
\end{description}
%--------------------------------------------------------------------------------------------------
\section{Commands}
...
\begin{description}
  ...
  \item[SET] -- Sets certain simulator options.
...
\begin{description}
  ...
  \begin{description}
    \item[8080] -- The Intel 8080 processor.  An 8 bit processor that can address up to 64kBytes of memory and 256 I/O ports.
    \item[8085] -- Made by Intel.  Very similar to the 8080 with mostly hardware enhancements, with 2 added instructions.
    \item[Z80] -- Made by Zilog.  It is mostly upward compatible with the 8080 (one flag is different on some instructions).  The added instruction codes for the 8085 are completely different instructions on the Z80.
    \item[68000] -- The Motorola 68000 processor.  This is internally a 32 bit processor with a 16 bit data bus, and 24 address lines.  It can address up to 16MBytes of memory.
    \item[68008] -- A stripped down version of the 68000.  The core is the same, but it has an 8 bit address bus and only 20 address lines.
    \item[PDP-11/04] -- A version of the PDP-11 series of minicomputers.  Currently, this is a placeholder.
    \item[PDP-11/05] -- A version of the PDP-11 series of minicomputers.
    ...
    \item[PDP-11/35] -- Under development.
    \item[PDP-11/40] -- The same as PDP-11/40.
  \end{description}
...
\subsection{Sim-CPU}
\subsubsection{Inputs}
A string containing the CPU name.  See the ``SET CPU'' (section \ref{cmd-set}) command for valid CPU names.
\subsubsection{Outputs}
None
\subsubsection{Description}
